...
//The Referendum count is related to the Governance aspect of Polkadot//, where in order to make any changes to the network, the idea is to //compose active token holders and the council together to administrat//e a network upgrade decision. No matter whether the proposal is put //forth //by the public (DOT holders) or the council, it will have to //go through// a referendum to let all DOT holders, weighted by stake,// enact the dec//ision. In many cases for new accounts, this will be //firmly at 0.    
    ...
    //api.rpc.chain.getBlock() queries the block hash at a specified block height, we then format the resulting block hash data for readability using .toHuman()
    const block = await api.rpc.chain.getBlock(blockHash);
    console.log('Block details: ', block.toHuman());
    
    block.block.extrinsics.forEach((extrinsic, index) => {
	// isSigned returns a boolean after checking whether the underlying API command(often referred to as an 'extrinsic') has an associated signature from a valid keypair. In this way, we can then log and view the encoded data from the blockchain in a Human-readable format
